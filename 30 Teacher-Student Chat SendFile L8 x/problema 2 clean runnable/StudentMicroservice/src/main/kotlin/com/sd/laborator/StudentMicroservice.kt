...
class StudentMicroservice {
    ...
    init {
        val databaseLines: List<String> = File("/home/stefan/Documents/Rezolvari Subiecte/30 Teacher-Student Chat SendFile L8 x/problema 2 clean runnable/StudentMicroservice/questions_database.txt").readLines()
    ...
    private fun subscribeToFileManager() {
        try {
            fileManagerSocket = Socket("localhost", 1900)
            println("M-am conectat la FileManager!")
        } catch (e: Exception) {
            println("Nu ma pot conecta la FileManager!")
            exitProcess(1)
        }
    }
    ...
    public fun run() {
        ...
        val fileBufferedReader = BufferedReader(InputStreamReader(fileManagerSocket.inputStream))
        ...
        while (true) {
            ...
            GlobalScope.launch {
                val response = fileBufferedReader.readLine()

                if (response == null) {
                    // daca se primeste un mesaj gol (NULL), atunci inseamna ca cealalta parte a socket-ului a fost inchisa
                    println("Microserviciul FileManager (${fileManagerSocket.port}) a fost oprit.")
                    bufferReader.close()
                    fileManagerSocket.close()
                    //break
                }

                // se foloseste un thread separat pentru tratarea intrebarii primite
                GlobalScope.launch {
                    val (messageType, file_name, file_body) = response.split(" ", limit = 3)

                    when(messageType) {
                        "file" -> {
                            // messageDestination = nume_fisier
                            // messageBody = content_fisier
                            println("Am primit un fisier: \"${file_name}\"")
                            File("/home/stefan/Documents/Rezolvari Subiecte/30 Teacher-Student Chat SendFile L8 x/problema 2 clean runnable/StudentMicroservice/$file_name").writeText(file_body.replace(" <ENTER> ", "\n"))
                        }
                    }
                }
            }
