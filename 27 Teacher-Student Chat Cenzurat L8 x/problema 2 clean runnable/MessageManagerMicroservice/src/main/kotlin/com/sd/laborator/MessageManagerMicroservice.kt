...
class MessageManagerMicroservice {
    ...
    {
        val censorSocket = Socket("localhost", 1900)
        censorSocket.getOutputStream().write((message+"\n").toByteArray())
        val response = BufferedReader(InputStreamReader(censorSocket.inputStream)).readLine()
        censorSocket.close()
        return response
    }
    ...
    private fun broadcastMessage(message: String, except: Int) {
        subscribers.forEach {
            it.takeIf { it.key != except }
                ?.value?.getOutputStream()?.write((censor(message) + "\n").toByteArray())
        }
    }

    private fun respondTo(destination: Int, message: String) {
        subscribers[destination]?.getOutputStream()?.write((censor(message) + "\n").toByteArray())
    }
