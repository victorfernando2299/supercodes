...
class BiddingProcessor:
    def __init__(self):
        ...
        self.loggerConsumer = RabbitPC.ConsumerLogger(config_logger)
    ...
    def run(self):
        threading.Thread(target=self.loggerConsumer.consume, args=()).start() # bidding processor trebuie deschis imediat dupa logger
        self.get_processed_bids()
        # aici se termina licitatia
        self.loggerProducer.send("gata")
