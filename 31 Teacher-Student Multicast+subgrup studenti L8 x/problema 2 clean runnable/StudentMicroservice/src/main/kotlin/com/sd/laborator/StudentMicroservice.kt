...
class StudentMicroservice {
    ...
    private fun subscribeToGroupManager() {
        try {
            groupManagerSocket = Socket("localhost", 1900)
            println("M-am conectat la GroupManager!")
        } catch (e: Exception) {
            println("Nu ma pot conecta la GroupManager!")
            exitProcess(1)
        }
    }
    ...
    public fun run() {
        ...
        val groupBufferedReader = BufferedReader(InputStreamReader(groupManagerSocket.inputStream))

        // se asteapta intrebari trimise prin intermediarul "MessageManager"

        GlobalScope.launch {
            while (true) {
                val response = groupBufferedReader.readLine()
                if (response == null) {
                    // daca se primeste un mesaj gol (NULL), atunci inseamna ca cealalta parte a socket-ului a fost inchisa
                    println("Conexiune refuzata in grup pe port (${groupManagerSocket.port})")
                    groupBufferedReader.close()
                    groupManagerSocket.close()
                    break
                }
                ...
                GlobalScope.launch {
                    ...
                    when (messageType) {
                        ...
                        "intrebare" -> {
                            println("GRUP: Am primit o intrebare de la $messageDestination: \"${messageBody}\"")
                            ...
                            responseToQuestion?.let {
                                ...
                                groupManagerSocket.getOutputStream().write((responseToQuestion + "\n").toByteArray())
                            }
                        }
                    }
                }
        ...
        while(true) // blocheaza thread-ul principal pt a nu se inchide programul (join pt corutine)
        {}
    }
}
